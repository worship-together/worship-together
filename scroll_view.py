...
def draw_notes(self, voice, measures, clef, tail_direction):
	global origin, end
	position = origin
	for measure in measures:
		...
		for note in voice_selected:
			...
			if clef == 'treble':
				self.note_dot = ui.Path.oval(position, C0_treble_y - (note_index * step), 3 * step, 2 * step)
				if tail_direction != 0:
					self.note_tail = ui.Path()
					self.note_tail.move_to(
						position + (((tail_direction * 1.5) + 1.5) * step), 
						(C0_treble_y - (note_index * step)) + step)
					self.note_tail.line_to(
						position + (((tail_direction * 1.5) + 1.5) * step),
						((C0_treble_y - (note_index * step)) + step) - (tail_direction * (7 * step)))
					self.note_tail.stroke()
			else:
				self.note_dot = ui.Path.oval(position, C0_bass_y - (note_index * step), 3 * step, 2 * step)
				if tail_direction != 0:
					self.note_tail = ui.Path()
					self.note_tail.move_to(
						position + (((tail_direction * 1.5) + 1.5) * step), 
						(C0_bass_y - (note_index * step)) + step)
					self.note_tail.line_to(
						position + (((tail_direction * 1.5) + 1.5) * step),
						((C0_bass_y - (note_index * step)) + step) - (tail_direction * (7 * step)))
					self.note_tail.stroke()
			self.note_dot.fill()
			ui.set_color('black')
...
class MusicView(ui.View):
	...
	def draw(self):
		...
		song = [song for song in midi.songs if song_number in str(song)][0]
		draw_notes(self, midi.Voice.Soprano, song.measures, 'treble', 1)
		draw_notes(self, midi.Voice.Alto, song.measures, 'treble', -1)
		draw_notes(self, midi.Voice.Tenor, song.measures, 'bass', 1)
		draw_notes(self, midi.Voice.Bass, song.measures, 'bass', -1)
